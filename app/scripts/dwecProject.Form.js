    ...
    $.DwecProject.Form = function (el, getData, options) {
        ...
        base.render = function (data) {
            ...
            $.each(restData, function (key) {
                ...
                obj['type'] == 'hidden' || obj['type'] == 'json' ? hidden = "hidden" : hidden = "";
        ...
        base.validate = function () {
            ...
            $.each(base.options.data.restData, function (key) {
                ...
                if (obj['uriData'] == null) {
                    ...
                    else if(obj['type'] == 'json' && input.val() == ""){
                        goodObject[obj['name']] = base.options.emptyJsonStructure;
                    }
                    else if(!(obj['type'] == 'json' && input.val() != "")) {
                        //NOTE: The reason of the inverse of this condition is:
                        // If we pass configuration to the api generates an 400 error because the attribute conceptType is an object an the api expects a key.
                        try {
                            goodObject[obj['name']] = JSON.parse(input.val());
                        } catch (e) {
            ...
            if (data != undefined) {
                $.each(rest, function (key) {
                    if (data[rest[key]['name']] != null) {
                        //console.log(data);
                        //console.log(data[rest[key]['name']]);
                        if (rest[key]['type'] == 'optionList') {
                            $("#" + base.id + " ." + rest[key]['name']).addClass('btn-success').text(data[rest[key]['name']]);
                        }
                        else if ($.isPlainObject(data[rest[key]['name']])) {
                            $("#" + base.id + " ." + rest[key]['name']).val(JSON.stringify(data[rest[key]['name']]));
                        }
                        else {
    ...
    $.DwecProject.Form.defaultOptions = {
        ...
        htmlRender: {
            ...
            fieldLabel: function (classes, labelName) {
                return "<div class='fieldLabel {classes}'>{labelName}</div>".format({
                    classes: classes,
                    labelName: labelName
                })
            },
