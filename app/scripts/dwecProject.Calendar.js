 ...
 * @authorLoadForm Dani Perhcés
 *
 * This plugin allows you to view events, upload on the server, on a calendar.
 * On the calendar we can create new events, delete or edit events with the help of a Form plugin.
 * Moreover we have different views on the calendar, view week in which we can modify the event time of a quick and
 * easy way or the month view in which we can also change day or expand dragging events.
 * It also has a column where we can show our generic events and drag the calendar if desired.
 *
 * Configurable parameters.
 * - Number of instances of the plugin
 * - All onError or onSuccess messages that you may occur.
 * - Possible texts that may appear
 * - Colors of events and trash
 * - Hours considers all day.
 * - Name of the generic event considered
 * - Name of the events we want to display on the calendar
 * - Urls host where to load events to interact with them.
 */
    ...
    base.addEvent = function () {
      ...
      base.$addEventBtn.on("click", function (event) {
        ...
        var fOnSuccessCallback = function fOnSuccessCallback(response) {
          ...
          if (base.options.dataFilters.model.indexOf(response.eventType.itemLabel)>-1){
            base.renderGenericEvents(response);
          }else if(base.options.dataFilters.shown.indexOf(response.eventType.itemLabel)>-1){
            base.$calendar.fullCalendar('renderEvent', myEvent);
          }
