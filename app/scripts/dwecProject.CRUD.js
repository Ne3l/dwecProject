(function ($) {
  ...
  $.Dwec.CRUD = function (el, getData, options) {
    ...
    base.addEvents = function () {
      $("#add_" + base.id).click(function () {
        base.doAdd();
      });

      $("#table_" + base.id).on('click', 'span[data-action = "set"]', function () {
        base.doUpdate($(this).siblings("input").val());
      });
    ...
    //CRUD Methods
    base.doAdd = function () {
		toastr.warning("ADD NOT IMPLEMENTED YET!!!!");
    };

    base.doUpdate = function (o) {
      //TODO update action
      toastr.warning("UPDATE NOT IMPLEMENTED YET!!!!");
      console.log(o);
    };
    ...
    //AJAX
    base.doProcessRequest = function (url, method, callbackSuccess) {
      $.ajax({
        method: method,
      ...
      for (var i = 0; i < base.actionsArray.length; i++) {
        if (base.actionsArray[i].type == "button") {
          ...
          buttons += base.format(base.options.templates.button, {
            ...
            action: base.actionsArray[i].linkType,
  ...
  $.Dwec.CRUD.defaultOptions = {
    ...
    templates: {
      ...
      button: "<button id = '{id}' class='btn sbold green' data-action='{action}' data-uri='{uri}'>{text} <span class='{icon}'></span></button>",
      input: "<input type='{type}' value ='{value}'>",
      divForm: "<div data-action='form'></div>"
    },
    classes: {
